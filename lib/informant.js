...
var init = function(store) {
	...
	io.sockets.on('connection',function(socket) {
		...
		socket.on('QUERY',function(msg) {
			//TODO: break out into it's own method
			...
			if(err) {
				socket.emit('QUERY_RESPONSE',{error:err});
				return;
			}
			...
			store.find(query,function(err,item) {
				if(err == null && item != null) {
					socket.emit('QUERY_RESPONSE',item);
				} else {
					socket.emit('QUERY_RESPONSE',{last:true,error:err});
				}
